 ...
 * @return boolean
 */
function isntPresent(obj): boolean {
	return obj === null || obj === undefined;
}
...
export class JsonFormValidatorService {
	 ...
	 * @param {String} path - JSON Schema path.
	 */
	private static checkProperties(schema, path: string) {
	...
	private static getParentGroup(path: string, level: any): FormGroup {
		...
		arr.forEach(item => {
			if (isPresent(level.controls[item])) {
				...
				JsonSchema2ReactiveFormService.getParentGroup(path, level);
			}
		});
	...
	private processSchema(schema: any, uiSchema: any = null, path = '', formLevel?: FormGroup): FormGroup {
		...
		if (schema.type === 'object') {
			JsonSchema2ReactiveFormService.checkProperties(schema, path);

			if (path) {
				const level = JsonSchema2ReactiveFormService.getParentGroup(path, formLevel);
