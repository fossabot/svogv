 ...
 */
export class DemoTabData {

  constructor(items: Array<DemoTab>) {
    this.tabs = items;
  }
...
export class DemoTabsComponent {
  ...
  @Input() tabs: DemoTabData;
  @Input() limitBreadcrumb = false;
  @Output() currentTab: DemoTab;
  ...
  private recurseRouteChildren(currentRoute: ActivatedRoute): void {
    ...
    childrenRoutes.forEach(route => {
      ...
      // verify that the custom data property "breadcrumb" is specified on the route,
      // limitBreadcrumb must be set to true to activate this
      if (!route.snapshot.data.hasOwnProperty(DemoTabsComponent.ROUTE_DATA_BREADCRUMB) && this.limitBreadcrumb) {
        return;
      }
